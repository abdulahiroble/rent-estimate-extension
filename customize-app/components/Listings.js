...
const Listings = ({ listings }) => {
    ...
    if (isLoading) return (
        <SkeletonTheme baseColor="#275F86" highlightColor='#256C9B' height={100}>
            <Skeleton />
        </SkeletonTheme>
    );
    ...
    if (selectedListing) {
        return <ListingDetails listing={selectedListing} onBack={() => setSelectedListing(null)} />;
    }

    return (
        <div>
            <Container py={8} px={0} maxW="container.md">
                <Carousel gap={32}>
                    {data && data.map((listing, index) => (
                        ...
                        >
                            <VStack align="stretch" spacing={2} mb={6}>
                                <Heading fontSize={{ base: "lg", md: "xl" }} textAlign="left" w="full" noOfLines={1}>
